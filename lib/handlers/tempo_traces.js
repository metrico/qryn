...
async function handler (req, res) {
  ...
  try {
    ...
    struct.resourceSpans.push.apply(struct.resourceSpans,
      resp.v2.map(span => formatSpanV2(span)))
...
function formatSpanV2 (span) {
  const res = {
    traceId: span.traceId ? Buffer.from(span.traceId, 'hex').toString('base64') : undefined,
    spanId: span.spanId ? Buffer.from(span.spanId, 'hex').toString('base64') : undefined,
    parentSpanId: span.parentId ? Buffer.from(span.parentId, 'hex').toString('base64') : undefined,
    name: span.name || '',
