...
const samplesThrottler = new TimeoutThrottler(
    `INSERT INTO ${clickhouseOptions.queryOptions.database}.${samplesTableName}(fingerprint, timestamp_ms, string) FORMAT JSONEachRow`)
const metricsThrottler = new TimeoutThrottler(
  `INSERT INTO ${clickhouseOptions.queryOptions.database}.metrics (fingerprint, timestamp_ms, value) FORMAT JSONEachRow`)
...
const onStale = function (data) {
  for (const entry of data.records.entries()) {
    const value = entry[1]
    samplesThrottler.queue.push.apply(samplesThrottler.queue, value.list.filter(v => v.record[3]).map(r => JSON.stringify({
      fingerprint: r.record[0],
      timestamp_ms: r.record[1],
      string: r.record[3]
    })))
    metricsThrottler.queue.push.apply(metricsThrottler.queue, value.list.filter(v => v.record[2] !== null).map(r => JSON.stringify({
      fingerprint: r.record[0],
      timestamp_ms: r.record[1],
      value: r.record[2]
    })))
...
const initialize = function (dbName) {
  ...
  ch.query(dbQuery, undefined, async function (err/*, data */) {
    ...
    let metricsTable = 'CREATE TABLE IF NOT EXISTS ' + dbName + '.metrics (fingerprint UInt64,timestamp_ms Int64 CODEC(DoubleDelta), value Float64 CODEC(Gorilla)) ENGINE = MergeTree PARTITION BY toRelativeHourNum(toDateTime(timestamp_ms / 1000)) ORDER BY (timestamp_ms)'
    ...
    await ch.query(metricsTable, undefined, function (err) {
      if (err) { console.log(err); process.exit(1) } else if (debug) console.log('Samples Table ready!')
      return true
    })
    ...
    if (storagePolicy) {
      ...
      const alterM = `ALTER TABLE ${dbName}.metrics MODIFY SETTING storagePolicy='${storagePolicy}'`
      ...
      await ch.query(alterM, undefined, function (err/*, data */) {
        if (err) { console.log(err) } else if (debug) console.log('Storage policy update for samples ' + storagePolicy)
        return true
      })
    }
