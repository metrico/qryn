...
const { samplesTableName } = require('./clickhouse')
...
module.exports = class extends EventEmitter {
  ...
  async initQuery () {
    const capabilities = await this.client.checkCapabilities()
  ...
  async initQueryWatchPoll () {
    ...
    await this.client.createLiveView(name, request.query, { timeout_sec: 10 })
    this.flushInterval = setInterval(this.flush.bind(this), 1000)
    while (this.working) {
      const [promise, cancel] = await this.client.watchLiveView(name, undefined,
  ...
  async initQueryCBPoll () {
    ...
    while (this.working) {
      ...
      await this.client.queryFingerprintsScan(this.query, {
        res: this
      })
      this.from = this.to
