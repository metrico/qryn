...
function handler (req, res) {
  ...
  if (streams) {
    streams.forEach(function (stream) {
      ...
      try {
        var tags = stream.tags ? Object.keys(stream.tags).map(function(key){ return {"key": key, "value": typeof stream.tags[key] == "string" ? stream.tags[key] : parseInt(stream.tags[key]), "type": typeof stream.tags[key] == "string" ? "string" : "int64"}; }) : [];
        var tempo = {
          "traceID": stream.traceId,
          "spanID": stream.id,
          "operationName": stream.name,
          "references": stream.references || [],
          "startTime": stream.timestamp,
          "duration": parseInt(stream.duration) || 100000,
          "tags": tags || [],
          "logs": [],
	...
	tempo = JSON.parse(JSON.stringify(tempo));
      ...
      console.log('store', finger, values);
