...
const scanTempo = async function (query, res) {
  ...
  const _query = transpiler.transpile(query)
  _query.step = UTILS.parseDurationSecOrDefault(query.step, 5) * 1000
  return queryTempoScan(_query, res)
}
...
const queryTempoScan = async function (query, res) {
  ...
  const _stream = await getClickhouseStream(query)
  const dataStream = preprocessStream(_stream, query.stream || [])
  return outputQueryTempo(dataStream, res, query.step, query.duration)
}
...
const outputQueryTempo = async (dataStream, res, i) => {
  ...
  for await (const item of gen()) {
    ...
    res.res.write(i ? ',' : '')
    res.res.write(JSON.stringify(entry))
    i++
  }
  res.res.write(']}')
  res.res.end()
