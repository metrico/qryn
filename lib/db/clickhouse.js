...
var reloadFingerprints = function(){
  ...
  var select_query = `SELECT DISTINCT fingerprint, labels FROM ${process.env.CLICKHOUSE_DB}.time_series`;
...
var scanFingerprints = async function (query, res) {
	 ...
	 *
	 * @param type {string}
	 * @param entry {{stream: any, values: any, metric: any}}
	 * @param last {boolean}
	 * @param emit {Function}
	 */
	const emitResult = (type, entry, last, emit) => {
		if (i === 0) {
			emit(`{"status":"success", "data":{ "resultType":"${type}", "result": [`); //"streams"
		}
		if (entry.values.length) {
			emit((i <= 1 ? "" : ",") + JSON.stringify(entry));
		}
		if (last) {
			emit("]}}");
		}
	...
	/**
	 *
	 * @param s {StringStream}
	 */
	let process = (s) => s
	if (!_query.matrix) {
		process = (s) => s.remap((emit, chunk) => {
			const row = chunk ? JSON.parse(chunk) : {};
			if (row.labels !== lastLabel) {
				emitResult("streams", {
					stream: JSON.parse(lastLabel),
					values: stream
				}, !chunk, emit);
				stream = [];
			}
			lastLabel = row.labels;
			stream.push([(parseInt(row.timestamp_ms) * 1000000).toString(), row.string]);
		});
	} else {
		...
		process = (s) => s.remap((emit, chunk) => {
			const row = chunk ? JSON.parse(chunk) : {};
			if (hashLabels(row.labels) !== hashLabels(lastLabel)) {
				if (hashLabels(lastLabel) && stream.length === 1) {
	...
	process(StringStream.from(_stream.data).lines())
		.map((c) => res.res.write(c))
		.run().then(() => res.res.end());
...
var scanMetricFingerprints = function(settings,client,params){
	...
	if (!settings.timefield) settings.timefield = process.env.TIMEFIELD || "timestamp_ms";
