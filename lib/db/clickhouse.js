...
const clickhouse_options = {
    ...
    protocol: process.env.CLICKHOUSE_PROTO+':' || 'http:',
...
class TimeoutThrottler {
    ...
    async flush() {
        ...
        const ts = Date.now();
        const  resp = await axios.post(`${protocol}://${clickhouse_options.auth}@${clickhouse_options.host}:${clickhouse_options.port}/?query=${this.statement}`,
            this.queue.join("\n")
        );
...
var scanFingerprints = async function (query, res) {
	...
	const _stream = await axios.post(`${protocol}://${clickhouse_options.auth}@${clickhouse_options.host}:${clickhouse_options.port}/`,
