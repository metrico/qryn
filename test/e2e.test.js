...
it('e2e', async () => {
  ...
  adjustMatrixResult(resp, testID)
  expect(resp.data).toMatchSnapshot()
  resp = await runRequest(`{test_id="${testID}_logfmt"}| line_format "{{ divide test_id 2  }}"`)
  expect(resp.data).toMatchSnapshot()
  resp = await runRequest(`rate({test_id="${testID}_logfmt"}| line_format "{{ divide int_lbl 2  }}" | unwrap _entry [1s])`)
  adjustMatrixResult(resp, testID + '_logfmt')
  expect(resp.data).toMatchSnapshot()
