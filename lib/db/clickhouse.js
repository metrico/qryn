...
const { parseLabels, hashLabels, isOmitTablesCreation } = require('../../common')
...
const scanTempo = async function (query, res) {
  return queryTempoScan(query, res)
}
...
const tempoQueryScan = async function (query, res, traceId) {
  ...
  const oldFrom = Date.now() - (this.tempo_span || 24) * 3600 * 1000
  response.v2 = await tempoQueryScanV2(query, res, traceId)
  if (!samplesReadTable.checkVersion('tempo_traces_v2', oldFrom)) {
    response.v1 = await tempoQueryScanV1(query, res, traceId)
  }
  return response
}

const tempoQueryScanV1 = async function (query, res, traceId) {
 ...
 */
const queryTempoScan = async function (query, res) {
  const resp = {
    v1: [],
    v2: []
  }
  resp.v2 = await queryTempoScanV2({...query}, res)
  if (!samplesReadTable.checkVersion('tempo_traces_v2', parseInt(query.start) / 1000000)) {
    resp.v1 = await queryTempoScanV1(query, res)
  }
  return resp
}

const queryTempoScanV1 = async function (query, res) {
  logger.debug('Scanning Fingerprints...')
  ...
  const _query = transpiler.transpile(query)
  _query.step = UTILS.parseDurationSecOrDefault(query.step, 5) * 1000
  const _stream = await getClickhouseStream(_query)
  const dataStream = preprocessStream(_stream, query.stream || [])
  return outputQueryTempo(dataStream, res, _query.step, _query.duration)
}

const queryTempoScanV2 = async function (query, res) {
  ...
  if (query.tags) {
    idxSubsel = Object.entries(query.tags)
      .map(e => {
        let subQ = `SELECT trace_id, span_id, timestamp_ns FROM ${DATABASE_NAME()}.tempo_traces_attrs_gin WHERE oid='0'` +
          ` AND date >= '${formatISO9075(query.start / 1000000).substring(0, 10)}' ` +
          ` AND date <= '${formatISO9075(query.end / 1000000).substring(0, 10)}'` +
          ` AND key = ${csql.quoteVal(e[0])} AND val = ${csql.quoteVal(e[1])}` +
  ...
  const resp = await axios.post(getClickhouseUrl() + '/', sql)
...
const outputQueryStreamsCSV = async (dataStream, res, i) => {
  res.res.writeHead(200, { 'Content-Type': 'text/csv', 'Access-Control-Allow-Origin': CORS })
  const gen = dataStream.toGenerator()
  const writer = createCsvArrayWriter({
    header: ['timestamp_ns', 'labels', 'string']
  })
  res.res.onBegin
    ? res.res.onBegin(writer.getHeaderString())
    : res.res.write(writer.getHeaderString())
  for await (const item of gen()) {
    if (!item) {
      continue
    }
    ...
    res.res.write(writer.stringifyRecords([record]))
  }
  res.res.onEnd ? res.res.onEnd('') : res.res.write('')
  res.res.end()
}
 ...
 */
const outputQueryTempo = async (dataStream, res, i) => {
  const _res = []
  const gen = dataStream.toGenerator()
  for await (const item of gen()) {
    if (!item) {
    ...
    let oItem = null
    try {
      oItem = JSON.parse(item.string)
    } catch (e) {
      continue
    }
    const serviceName = oItem.localEndpoint && oItem.localEndpoint.serviceName ? oItem.localEndpoint.serviceName : ''

    _res.push({
      traceID: oItem.traceID,
      rootServiceName: serviceName,
      rootTraceName: oItem.name || '',
      startTimeUnixNano: oItem.startTimeUnixNano || '',
      durationMs: ((oItem.endTimeUnixNano || 0) - (oItem.startTimeUnixNano || 0)) / 1e6
    })
  }
  return _res
}
...
const outputQueryMatrixCSV = async (dataStream, res,
  ...
  for await (const item of gen()) {
    ...
    for (let j = 0; j < addPoints; ++j) {
      ...
      ts && res.res.write(writer.stringifyRecords([record]))
