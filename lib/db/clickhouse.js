...
const queryTempoScan = async function (query) {
  ...
  if (!samplesReadTable.checkVersion('tempo_traces_v2', parseInt(query.start) / 1000000)) {
    resp.v1 = await queryTempoScanV1(query)
  }
  return resp
}

const queryTempoScanV1 = async function (query) {
  logger.debug('Scanning Fingerprints...')
  ...
  const dataStream = preprocessStream(_stream, query.stream || [])
  return outputQueryTempo(dataStream, _query.step, _query.duration)
}
 ...
 */
const outputQueryTempo = async (dataStream) => {
  const _res = []
  const gen = dataStream.toGenerator()
  for await (const item of gen()) {
    if (!item) {
