...
function processStream (stream, labels, bulkLabels, bulkValues, toJSON, fingerPrint) {
  let finger = null
  let JSONLabels
  const promises = []
  if (stream.stream) {
  ...
  finger = fingerPrint(JSON.stringify(JSONLabels))
  // Store Fingerprint
  ...
  if (stream.entries) {
    ...
    promises.push(bulkValues.add(values))
  }
  if (stream.values) {
    const values = []
...
class ConditilnalStreamValues extends StreamValues {
  constructor (options) {
    super()
...
const processRawPush = async (req, labels, bulkLabels, bulkValues, toJSON, fingerPrint) => {
  ...
  const pipeline = chain([
    ...
    new ConditilnalStreamValues({ filter: /^streams\.\d+\.stream/, id: 'stream' }),
    new ConditilnalStreamValues({ filter: /^streams\.\d+\.values\.\d+/, id: 'values' })
  ])
  let size = 0
