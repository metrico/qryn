...
// TODO: add Diff query method (only necessary for diff view which is not available in oss grafana, the schema should be enough and backward compatible)

async function selectPyroscopeProfileTypes(query) {
  // group by instead distinct on LowCardinality col for optimization: https://github.com/ClickHouse/ClickHouse/issues/4670
  // also group by is better for multi value distinct query: https://github.com/ClickHouse/ClickHouse/issues/17916
  const q = `SELECT splitByString(':', type_id)
    FROM ${DB}.pyroscope_profiles 
    WHERE oid='0' AND timestamp_ns >= ${toTimestampNsQueryExpr(query.start)} AND timestamp_ns <= ${toTimestampNsQueryExpr(query.end)}
    GROUP BY type_id
    FORMAT JSON`
  const resp = await axios.post(getClickhouseUrl() + '/', q)
  return resp.data.data ? resp.data.data : JSON.parse(resp.data).data
...
function toTimestampNsQueryExpr(datetime) {
  return `toUnixTimestamp(${datetime}) * 1000000000`
}

async function selectPyroscopeTags(query) {
  const idxSubsel = query.matchers ? `AND profile_id GLOBAL IN (${toUnionGinIndexSubselect(query.matchers)})` : ''
  const q = `SELECT key
    FROM ${DB}.pyroscope_profiles_attrs_gin 
    WHERE oid='0' AND date >= '${formatISO9075(query.start).substring(0, 10)}' AND date <= '${formatISO9075(query.end).substring(0, 10)}'
    ${idxSubsel}
    GROUP BY key
    FORMAT JSON`
  const resp = await axios.post(getClickhouseUrl() + '/', q)
...
async function selectPyroscopeTagValues(query) {
  const idxSubsel = query.matchers ? `AND profile_id GLOBAL IN (${toUnionGinIndexSubselect(query.matchers)})` : ''
  const q = `SELECT val
    FROM ${DB}.pyroscope_profiles_attrs_gin 
    WHERE oid='0' AND key = '${query.tag}' AND date >= '${formatISO9075(query.start).substring(0, 10)}' AND date <= '${formatISO9075(query.end).substring(0, 10)}'
    ${idxSubsel}
    GROUP BY val
    FORMAT JSON`
  const resp = await axios.post(getClickhouseUrl() + '/', q)
...
async function selectPyroscopeKVSet(query) {
  const idxSubsel = query.matchers ? `AND profile_id GLOBAL IN (${toUnionGinIndexSubselect(query.matchers)})` : ''
  const q = `SELECT key, val
    FROM ${DB}.pyroscope_profiles_attrs_gin 
    WHERE oid='0' AND date >= '${formatISO9075(query.start).substring(0, 10)}' AND date <= '${formatISO9075(query.end).substring(0, 10)}'
    ${query.names ? ` AND key IN (${query.names.map(n => csql.quoteVal(n)).join(' AND ')})` : ''} 
    ${idxSubsel}
    GROUP BY key, val
    FORMAT JSON`
  const resp = await axios.post(getClickhouseUrl() + '/', q)
...
async function selectPyroscopeTotalTimeSeries(query) {
  // TODO: add groupby array option by joining the gin table and doing group by iteratively, limit results like: LIMIT ${query.limit * query.maxTagsPerTimeSeries} where query.maxTagsPerTimeSeries = query.maxTagsPerTimeSeries > 0 ? query.maxTagsPerTimeSeries : 30
  query.limit = query.limit > 0 ? query.limit : 10000
  let idxSubsel = query.tags ? `AND profile_id GLOBAL IN (${toGinIndexSubselect(query.tags)})` : ''
  const q = `SELECT toStartOfInterval(toDateTime(intDiv(pyroscope_profiles.timestamp_ns, 1000000000)), INTERVAL ${query.step ? query.step : 60} SECOND) AS bucket_time,
    sum(pyroscope_profiles.value) AS value
    FROM ${DB}.pyroscope_profiles
    WHERE oid='0' AND timestamp_ns >= ${toTimestampNsQueryExpr(query.interval.start)} AND timestamp_ns <= ${toTimestampNsQueryExpr(query.interval.end)}
    AND type_id = '${query.type_id}'
    ${idxSubsel}
    GROUP BY bucket_time
    ORDER BY bucket_time DESC`
  const resp = await rawRequest(q, null, DATABASE_NAME())
  return resp.data.data ? resp.data.data : JSON.parse(resp.data).data
}

async function selectPyroscopeProfiles(query) {
  query.limit = query.limit > 0 ? query.limit : 2000
  const idxSubsel = query.tags ? `AND profile_id GLOBAL IN (${toGinIndexSubselect(query.tags)})` : ''
  const q = `SELECT hex(profile_id) as profile_id_hex, duration_ns, value, payload_type, payload
    FROM ${DATABASE_NAME()}.pyroscope_profiles
    WHERE oid='0' AND timestamp_ns >= ${toTimestampNsQueryExpr(query.interval.start)} AND timestamp_ns <= ${toTimestampNsQueryExpr(query.interval.end)}
    AND type_id = '${query.type_id}' 
    ${idxSubsel}
...
function toUnionGinIndexSubselect(tagMatrix) {
  return `SELECT profile_id FROM (
    ${tagsMatrix.map(arr => {
      let subSubSel = `SELECT profile_id
        FROM ${toGinIndexSubselect(arr)}`
      return subSubSel
    }).join(' UNION ')}) as multisubsel
    ORDER BY date DESC 
    LIMIT ${query.limit}`
}

function toGinIndexSubselect(tags) {
  return `SELECT profile_id FROM (
    ${Object.entries(query.tags)
      .map(e => {
        let subSubSel = `SELECT profile_id
          FROM ${DATABASE_NAME()}.pyroscope_profiles_attrs_gin 
          WHERE oid='0'
          AND date >= '${formatISO9075(query.start).substring(0, 10)}'
          AND date <= '${formatISO9075(query.end).substring(0, 10)}'
          AND key = ${csql.quoteVal(e[0].toString())} AND val = ${csql.quoteVal(e[1].toString())}`
        return subSubSel
      }).join(' INTERSECT ')}) as rawsubsel
    ORDER BY date DESC 
    LIMIT ${query.limit}`
}

async function insertPyroscopeProfiles(query) {
  return new Promise((resolve, reject) => {
    const id = getThrottlerId()
    throttler.postMessage({
      type: 'profiles',
      data: profiles.map(obj => (new Pyroscope(obj)).toJson()).join('\n'),
      id: id
    })
    resolvers[id] = resolve
    rejectors[id] = reject
...
const rawRequest = (query, data, database) => {
  ...
  return axios.post(url, data || query)
    .catch(e => {
      logger.error(e.response ? e.response.data : "Error response from clickhouse")
