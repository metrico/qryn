...
module.exports.rotate = async (opts) => {
  for (const db of opts) {
    ...
    if (db.samples_days + '' !== settings.samples_days) {
      ...
      await client.addSetting('rotate', 'samples_days', db.samples_days + '', db.db)
    }
    if (db.time_series_days + '' !== settings.time_series_days) {
      ...
      await client.rawRequest(alterTable, null, db.db)
      await client.rawRequest(rotateTable, null, db.db)
      await client.addSetting('rotate', 'time_series_days', db.time_series_days + '', db.db)
    }
    if (db.storage_policy && db.storage_policy !== settings.storage_policy) {
      ...
      await client.addSetting('rotate', 'storage_policy', db.storage_policy, db.db)
    }
  }
}
