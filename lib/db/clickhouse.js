 ...
 */
module.exports.getDatabaseIP = () => {
  return new Promise((resolve, reject) => {
    lookup(clickhouseOptions.host, (err, res) => {
      err && reject(err)
      resolve(res)
    })
  })
 ...
 *
 * @param name {string}
 * @returns {Promise<undefined|{name: string, request: string, labels: Object<string, string>}>}
 */
module.exports.getAlertRule = async (name) => {
  ...
  const res = await axios.post(getClickhouseUrl(),
    ...
    `WHERE fingerprint = ${UTILS.fingerPrint('alert_rule' + name)} AND ${mark} == ${mark} ` +
 ...
 */
module.exports.putAlertRule = async (name, query, labels) => {
  const value = JSON.stringify({ request: query, labels: labels })
    .replace(/\\/g, '\\\\')
    .replace(/'/g, '\\\'')
  name = name.replace(/\\/g, '\\\\')
    .replace(/'/g, '\\\'')
  ...
  await axios.post(getClickhouseUrl(),
    `INSERT INTO ${DATABASE_NAME()}.settings (fingerprint, type, name, value, inserted_at) VALUES ` +
    `(${fp}, 'alert_rule', '${name}', '${value}', now64(9, 'UTC'));`
  )
}
 ...
 * @returns {Promise<[{name: string, request: string, labels: Object<string, string>}]>}
 */
module.exports.getAlertRules = async (limit, offset, name) => {
  ...
  let query
  if (!name) {
    query = 'SELECT fingerprint, argMax(name, inserted_at) as name, argMax(value, inserted_at) as value ' +
    `FROM ${DATABASE_NAME()}.settings ` +
    `WHERE type == 'alert_rule' AND  ${mark} == ${mark} ` +
    `GROUP BY fingerprint HAVING name != '' ORDER BY name ${_limit} ${_offset} FORMAT JSON`
  } else {
    query = `SELECT * FROM ${DATABASE_NAME()}.settings ` +
    `WHERE type == 'alert_rule' AND ${mark} == ${mark} AND ilike(name, '%${name}%') ` +
    `ORDER BY name ${_limit} ${_offset} FORMAT JSON`
  }

  const res = await axios.post(getClickhouseUrl(), query)
  return res.data.data.map(e => {
    const rule = JSON.parse(e.value)
    return {
      ...rule,
      name: e.name
    }
 ...
 * @returns {Promise<undefined>}
 */
module.exports.deleteAlertRule = async (name) => {
  const fp = UTILS.fingerPrint('alert_rule' + name)
  await axios.post(getClickhouseUrl(),
    `INSERT INTO ${DATABASE_NAME()}.settings (fingerprint, type, name, value, inserted_at) VALUES ` +
    `(${fp}, 'alert_rule', '', '', now64(9, 'UTC'));`
  )
  await axios.post(getClickhouseUrl(),
...
module.exports.incAlertMark = async (name) => {
  const fp = UTILS.fingerPrint('alert_rule' + name)
  const mark = await axios.post(getClickhouseUrl(),
    `SELECT max(mark) as mark FROM ${DATABASE_NAME()}._alert_view_${fp}_mark FORMAT JSON`)
  await axios.post(getClickhouseUrl(),
    `INSERT INTO ${DATABASE_NAME()}._alert_view_${fp}_mark (mark) VALUES (${Date.now()})`)
  return parseInt(mark.data.data[0].mark)
}

module.exports.getLastAlert = async (name, mark) => {
  const fp = UTILS.fingerPrint('alert_rule' + name)
  const lastMsg = await axios.post(getClickhouseUrl(),
    `SELECT * FROM ${DATABASE_NAME()}._alert_view_${fp} WHERE mark <= ${mark} ORDER BY timestamp_ms DESC LIMIT 1 FORMAT JSON`)
  if (!lastMsg.data.data || !lastMsg.data.data.length) {
    return undefined
  }
  return lastMsg.data.data[0]
}

module.exports.dropOutdatedParts = async (name, mark) => {
  const fp = UTILS.fingerPrint('alert_rule' + name)
