...
const storage_policy = process.env.STORAGE_POLICY || false
...
class TimeoutThrottler {
  ...
  async flush () {
    ...
    const ts = Date.now()
    const resp = await axios.post(`${get_clickhouse_url()}/?query=${this.statement}`,
      this.queue.join('\n')
    )
...
const initialize = function (dbName) {
  ...
  const tmp = { ...clickhouse_options, queryOptions: { database: '' } }
  ...
  ch.query(dbQuery, undefined, async function (err, data) {
    ...
    const ch = new ClickHouse(clickhouse_options)
    hack_ch(ch)
    ...
    if (storage_policy) {
      console.log('ADD SETTINGS storage policy', storage_policy)
      const set_storage = ` SETTINGS storage_policy='${storage_policy}'`
    ...
    var rotate_table = 'ALTER TABLE ' + dbName + '.samples MODIFY TTL toDateTime(timestamp_ms / 1000)  + INTERVAL ' + rotation_samples + ' DAY'
      ...
      if (err) { console.log(err) } else if (debug) console.log('Samples Table rotation set to days: ' + rotation_samples)
      return true
    })
    ...
    var rotate_table = 'ALTER TABLE ' + dbName + '.time_series MODIFY TTL date  + INTERVAL ' + rotation_labels + ' DAY'
      ...
      return true
	  	...
	  	await ch.query(rotate_table, undefined, function (err, data) {
      if (err) { console.log(err) } else if (debug) console.log('Labels Table rotation set to days: ' + rotation_labels)
      return true
    })

    if (storage_policy) {
      console.log('ALTER storage policy', storage_policy)
      const alter_ts = `ALTER TABLE ${dbName}.time_series MODIFY SETTING storage_policy='${storage_policy}'`
      const alter_sm = `ALTER TABLE ${dbName}.samples MODIFY SETTING storage_policy='${storage_policy}'`

      await ch.query(alter_ts, undefined, function (err, data) {
        if (err) { console.log(err) } else if (debug) console.log('Storage policy update for fingerprints ' + storage_policy)
        return true
      })
      await ch.query(alter_sm, undefined, function (err, data) {
        if (err) { console.log(err) } else if (debug) console.log('Storage policy update for samples ' + storage_policy)
        return true
      })
    }
    ...
    /* TODO: tsv2
		const tsv2 = await axios.get(`${protocol}://${clickhouse_options.auth}@${clickhouse_options.host}:${clickhouse_options.port}/?query=SHOW TABLES FROM ${dbName} LIKE 'time_series_v2' FORMAT JSON`);
		if (!tsv2.data.rows) {
			...
			await axios.post(`${protocol}://${clickhouse_options.auth}@${clickhouse_options.host}:${clickhouse_options.port}/`,
				insert);
		} */
...
const checkCapabilities = async () => {
  ...
  try {
    await axios.post(get_clickhouse_url() + '/?allow_experimental_live_view=1',
			`CREATE LIVE VIEW ${clickhouse_options.queryOptions.database}.lvcheck WITH TIMEOUT 1 AS SELECT 1`)
...
var reloadFingerprints = function () {
  ...
  const select_query = `SELECT DISTINCT fingerprint, labels FROM ${clickhouse_options.queryOptions.database}.time_series`
...
function get_clickhouse_url () {
  return `${protocol}://${clickhouse_options.auth}@${clickhouse_options.host}:${clickhouse_options.port}`
}
...
var queryFingerprintsScan = async function (query, res) {
  ...
  const _stream = await axios.post(get_clickhouse_url() + '/',
...
const scanClickhouse = function (settings, client, params) {
  ...
  if (process.env.READONLY) {
	  const tmp = { ...clickhouse_options, queryOptions: { database: settings.db } }
	  ch = new ClickHouse(tmp)
  }
 ...
 * @param res {{res: {write: (function(string)), writeHead: (function(number, {}))}}}
 */
const get_series = async (matches, res) => {
  const query = transpiler.transpile_series(matches)
  const stream = await axios.post(`${get_clickhouse_url()}`, query + ' FORMAT JSONEachRow', {
    responseType: 'stream'
  })
...
const ping = async () => {
  await Promise.all([
    new Promise((f, r) => ch.query('SELECT 1', undefined, (err) => {
      err ? r(err) : f(err)
    })),
    axios.get(`${get_clickhouse_url()}/?query=SELECT 1`)
  ])
}
...
module.exports.create_live_view = (name, request, options) => {
  const db = options.db || clickhouse_options.queryOptions.database
  const timeout = options.timeout_sec ? `WITH TIMEOUT ${options.timeout_sec}` : ''
  return axios.post(`${get_clickhouse_url()}/?allow_experimental_live_view=1`,
...
module.exports.watch_live_view = async (name, db, res, options) => {
  db = db || clickhouse_options.queryOptions.database
  const cancel = axios.CancelToken.source()
  const stream = await axios.post(`${get_clickhouse_url()}/?allow_experimental_live_view=1`,
...
module.exports.scanSeries = get_series
