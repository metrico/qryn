...
var scanFingerprints = function(query, res){
	...
	const stream = ch.query(_query);
	stream.on('data', (row) => {
	    streams[row[2]] = streams[row[2]] || {
	        stream: JSON.parse(row[0]),
            values: []
        };
	    streams[row[2]].values.push([(parseInt(row[3]) * 1000000) + '', row[1]]);
    });
	stream.on('error', (e) => {
	    console.log(e);
