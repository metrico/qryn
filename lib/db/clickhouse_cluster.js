...
module.exports.singleNodeInit = async (dbName, _ch, suffix) => {
  ...
  const readTable = 'CREATE TABLE IF NOT EXISTS ' + dbName + `.${samplesReadTableName}${suffix} (fingerprint UInt64,timestamp_ms Int64,value Float64,string String) ENGINE=Merge(\'${dbName}\', \'^(samples|samples_v[0-9]+)$\')`
...
module.exports.clusterInit = async (dbName) => {
  ...
  const tsTable = 'CREATE TABLE IF NOT EXISTS ' + dbName + `.time_series as ${dbName}.time_series_standalone ENGINE = Distributed(${clusterName}, ${dbName}, time_series_standalone, rand());`
  const readTable = 'CREATE TABLE IF NOT EXISTS ' + dbName + `.${samplesReadTableName} as ${dbName}.${samplesReadTableName}_standalone ENGINE = Distributed(${clusterName}, ${dbName}, ${samplesReadTableName}_standalone, rand());`
