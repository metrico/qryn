...
const { flatOTLPAttrs } = require('../utils')
...
function formatOTLPSpanV2 (span, json) {
  ...
  for (const attr of ['peer.service', 'service.name', 'faas.name', 'k8s.deployment.name', 'process.executable.name']) {
    if (flatSpanAttrs[attr]) {
      serviceName = flatSpanAttrs[attr]
      break
    }
  }
  serviceName = serviceName || 'OTLPResourceNoServiceName'

  const res = {
    traceID: span.traceId,
    traceId: span.traceId ? getId(span.traceId, 32) : undefined,
    spanID: span.spanId,
    spanId: span.spanId ? getId(span.id, 16) : undefined,
    ...
    events: (span.events || []).map(a => ({
      ...
      name: stringify(flatOTLPAttrs([
        ...a.attributes, a.name ? { key: 'name', value: { stringValue: a.name } } : null
      ]))
    }))
  }
  return res
