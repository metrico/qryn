...
var initialize = function(dbName){
	...
	ch.query(dbQuery, undefined, async function (err, data) {
		...
		if (storage_policy){
			...
			var set_storage = ` SETTINGS storage_policy='${storage_policy}'`;
		...
		if (storage_policy){
			...
			var alter_ts = `ALTER TABLE ${dbName}.time_series MODIFY SETTING storage_policy='${storage_policy}'`;
			var alter_sm = `ALTER TABLE ${dbName}.samples MODIFY SETTING storage_policy='${storage_policy}'`;
			
			await ch.query(alter_ts, undefined, function(err,data){
				if (err) { console.log(err); }
				else if (debug) console.log('Storage policy update for fingerprints '+storage_policy);
				return true;
			});
			await ch.query(alter_sm, undefined, function(err,data){
				if (err) { console.log(err); }
				else if (debug) console.log('Storage policy update for samples '+storage_policy);
				return true;
			});
		}
...
const processResponseStream = async (query, _stream, res) => {
	...
	let dStream = preprocessStream(_stream, query.stream);
...
var scanClickhouse = function(settings,client,params){
	...
	if (process.env.READONLY){
	  var tmp = { ...clickhouse_options, queryOptions: { database: settings.db } };
	  ch = new ClickHouse(tmp);
	}
...
const get_series = async (matches, res) => {
	...
	let dStream = StringStream.from(stream.data).lines().map(l => {
		if (!l) {
			return null;
		}
		try {
