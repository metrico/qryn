...
const { clickhouseOptions: getClickhouseOptions, getAClient, init } = require('./clickhouse_cluster')

/* DB Helper */
const ClickHouse = require('@apla/clickhouse')
const clickhouseOptions = getClickhouseOptions()
...
const { isClustered } = require('../utils')
...
module.exports.createLiveView = (name, request, options) => {
  ...
  const timeout = options.timeout_sec ? `WITH TIMEOUT ${options.timeout_sec}${isClustered ? ' AND REFRESH 2' : ''}` : ''
