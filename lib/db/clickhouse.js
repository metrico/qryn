...
const initialize = function (dbName) {
  ...
  ch.query(dbQuery, undefined, async function (err/*, data */) {
    ...
    if (rotationSamples > 0) {
      let alterTable = 'ALTER TABLE ' + dbName + `.${samplesTableName} MODIFY SETTING ttl_only_drop_parts = 1, merge_with_ttl_timeout = 3600, materialize_ttl_after_modify = 1, index_granularity = 8192`
    ...
    if (rotationLabels > 0) {
      alterTable = 'ALTER TABLE ' + dbName + '.time_series MODIFY SETTING ttl_only_drop_parts = 1, merge_with_ttl_timeout = 3600,  materialize_ttl_after_modify = 1, index_granularity = 8192'
