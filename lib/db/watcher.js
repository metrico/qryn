...
const {queryFingerprintsScan, create_live_view, watch_live_view} = require("./clickhouse");
...
module.exports = class extends EventEmitter {
    ...
    initQuery() {
        ...
        return this.initQueryWatchPoll();
    }

    async initQueryWatchPoll() {
        this.watch = true;
        let request = transpiler.transpile_tail(this.request);
        const name = `watcher_${this.uid.toString().substr(2)}`;
        await create_live_view(name, request.query, {timeout_sec: 10});
        this.flushInterval = setInterval(this.flush.bind(this), 1000);
        while (this.working) {
            const [promise, cancel] = await watch_live_view(name, undefined,
