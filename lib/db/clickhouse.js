...
const tempoQueryScan = async function (query, res) {
  if (debug) console.log('Scanning Tempo Fingerprints...')
  const time = parseMs(query.time, Date.now())
  ...
  const dataStream = preprocessStream(_stream, _query.stream || [])
  return await (outputTempoSpans(dataStream, res))
}
 ...
 * @returns {Promise<void>}
 */
const outputTempoSpans = async (dataStream, res) => {
  res.res.writeHead(200, { 'Content-Type': 'application/json' })
  const gen = dataStream.toGenerator()
  let i = 0
  let lastLabels = null
  let lastStream = []
  res.res.write('{"total": 0, "limit": 0, "offset": 0, "errors": null, "data": [')
  for await (const item of gen()) {
    if (!item) {
      continue
    }
