...
const upgradeSingle = async (db, key, scripts) => {
  ...
  let ver = await client.rawRequest(`SELECT max(ver) as ver FROM {{DB}}.ver WHERE k = ${key} FORMAT JSON`,
    null, db)
...
const upgradeRequest = async (db, request, useDefaultDB, updateVer) => {
  ...
  if (updateVer) {
    console.log(`INSERT INTO {{DB}}.ver (k, ver) VALUES (${updateVer.key}, ${updateVer.to})`)
