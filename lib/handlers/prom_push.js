  ...
  if (streams) {
    streams.forEach(function (stream) {
      ...
      try {
        try {
          if (stream.labels) {
             stream.labels.forEach(function (tag) {
               Object.assign(JSONLabels, tag.replace(/\./g, '_'))
             })
          }
          if (JSONLabels.name) delete Object.assign(JSONLabels, {['__name__']: JSONLabels['name'] || "undefined" })['name'];
        } catch (err) {
          req.log.error({ err })
          return
        }
