...
module.exports.getAlertRule = async (name) => {
  const res = await axios.post(getClickhouseUrl(),
    ...
    `WHERE fingerprint = ${UTILS.fingerPrint('alert_rule' + name)} ` +
...
module.exports.createAlertViews = async (name, request) => {
  const fp = UTILS.fingerPrint('alert_rule' + name)
  request.select.push(`(SELECT max(mark) FROM ${DATABASE_NAME()}._alert_view_${fp}_mark) as mark`)
  const strRequest = transpiler.requestToStr(request)
  await axios.post(getClickhouseUrl(),
    `CREATE TABLE ${DATABASE_NAME()}._alert_view_${fp}_mark (id UInt8 default 0,mark UInt64) ` +
...
module.exports.incAlertMark = async (name) => {
  ...
  const mark = await axios.post(getClickhouseUrl(),
    `SELECT max(mark) FROM ${DATABASE_NAME()}._alert_view_${fp}_mark FORMAT JSON`)
  await axios.post(getClickhouseUrl(),
    `INSERT INTO ${DATABASE_NAME()}._alert_view_${fp}_mark (mark) VALUES (${Date.now()})`)
