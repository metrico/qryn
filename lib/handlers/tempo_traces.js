...
const TraceDataType = protoBuff.loadSync(path.join(__dirname, '/../opentelemetry/proto/trace/v1/trace.proto'))
...
async function handler (req, res) {
  ...
  /* remove newlines */
  if (this.tempo_tagtrace) req.query.query = `{traceId=${JSON.stringify(req.params.traceId)}}`
  else req.query.query = `{type="tempo"} |~ ${JSON.stringify(req.params.traceId)}`
  ...
  try {
    ...
    const resp = await client.tempoQueryScan(
    ...
    parsed.data[0].spans.forEach(function (span) {
      ...
      if (span.localEndpoint.serviceName || span.serviceName) {
        attributes.push({
          key: 'service.name',
          value: { stringValue: span.localEndpoint.serviceName || span.serviceName }
        })
      }
      if (span.traceID) {
