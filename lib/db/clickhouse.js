...
const conveyor = {
  ...
  stop: () => {
    conveyor.updater && clearInterval(conveyor.updater)
    conveyor.updater = null
  }
}
...
const scanFingerprints = async function (query, res) {
  ...
  if (!_query.matrix && conveyor.labels > 100000) {
    return await require('./labels_conveyor').queryRange(query, res)
  }
