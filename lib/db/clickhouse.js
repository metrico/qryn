...
const onStale = function (data) {
  for (const entry of data.records.entries()) {
    ...
    metricsThrottler.queue.push.apply(metricsThrottler.queue, value.list.filter(v => !v.record[3]).map(r => JSON.stringify({
      fingerprint: r.record[0],
      timestamp_ms: r.record[1],
      value: r.record[2]
    })))
...
const initialize = function (dbName) {
  ...
  ch.query(dbQuery, undefined, async function (err/*, data */) {
    ...
    let metricsTable = 'CREATE TABLE IF NOT EXISTS ' + dbName + '.metrics (fingerprint UInt64,timestamp_ms Int64,value Float64) ENGINE = MergeTree PARTITION BY toRelativeHourNum(toDateTime(timestamp_ms / 1000)) ORDER BY (timestamp_ms)'
