...
module.exports.rotate = async (opts) => {
  for (const db of opts) {
    const settings = await client.getSettings([
      { type: 'rotate', name: 'v4_samples_days' },
      { type: 'rotate', name: 'v4_time_series_days' },
      { type: 'rotate', name: 'v4_storage_policy' }
    ], db.db)
    if (db.samples_days + '' !== settings.v4_samples_days) {
      const alterTable = 'ALTER TABLE samples_v4 MODIFY SETTING ttl_only_drop_parts = 1, merge_with_ttl_timeout = 3600, index_granularity = 8192'
      const rotateTable = `ALTER TABLE samples_v4 MODIFY TTL toDateTime(timestamp_ns / 1000000000) + INTERVAL ${db.samples_days} DAY`
      await client.rawRequest(alterTable, null, db.db)
      await client.rawRequest(rotateTable, null, db.db)
      await client.addSetting('rotate', 'v4_samples_days', db.samples_days + '', db.db)
    }
    if (db.time_series_days + '' !== settings.v4_time_series_days) {
      ...
      await client.addSetting('rotate', 'v4_time_series_days', db.time_series_days + '', db.db)
    }
    if (db.storage_policy && db.storage_policy !== settings.v4_storage_policy) {
      ...
      await client.addSetting('rotate', 'v4_storage_policy', db.storage_policy, db.db)
    }
  }
}
