...
module.exports.rotate = async (opts) => {
  for (const db of opts) {
    ...
    if (db.samples_days + '' !== settings.v1_profiles_days) {
      let alterTable = 'ALTER TABLE profiles {{DB}}.{{{OnCluster}}} MODIFY SETTING ttl_only_drop_parts = 1, merge_with_ttl_timeout = 3600, index_granularity = 8192'
      let rotateTable = `ALTER TABLE profiles {{DB}}.{{{OnCluster}}} MODIFY TTL toDateTime(timestamp_ns / 1000000000) + INTERVAL ${db.samples_days} DAY`
      await _update(alterTable, null, db.db)
      await _update(rotateTable, null, db.db)
      alterTable = 'ALTER TABLE profiles_series {{DB}}.{{{OnCluster}}} MODIFY SETTING ttl_only_drop_parts = 1, merge_with_ttl_timeout = 3600, index_granularity = 8192'
      rotateTable = `ALTER TABLE profiles_series {{DB}}.{{{OnCluster}}} MODIFY TTL date + INTERVAL ${db.samples_days} DAY`
      await _update(alterTable, null, db.db)
      await _update(rotateTable, null, db.db)
      alterTable = 'ALTER TABLE profiles_series_gin {{DB}}.{{{OnCluster}}} MODIFY SETTING ttl_only_drop_parts = 1, merge_with_ttl_timeout = 3600, index_granularity = 8192'
      rotateTable = `ALTER TABLE profiles_series_gin {{DB}}.{{{OnCluster}}} MODIFY TTL date + INTERVAL ${db.samples_days} DAY`
      await _update(alterTable, null, db.db)
      await _update(rotateTable, null, db.db)
      alterTable = 'ALTER TABLE profiles_series_keys {{DB}}.{{{OnCluster}}} MODIFY SETTING ttl_only_drop_parts = 1, merge_with_ttl_timeout = 3600, index_granularity = 8192'
      rotateTable = `ALTER TABLE profiles_series_keys {{DB}}.{{{OnCluster}}} MODIFY TTL date + INTERVAL ${db.samples_days} DAY`
      await _update(alterTable, null, db.db)
      await _update(rotateTable, null, db.db)
