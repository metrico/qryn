...
var scanFingerprints = async function (query, res) {
	...
	if (!_query.matrix) {
		process = (s) => s.remap((emit, chunk) => {
			...
			if (row.labels !== lastLabel && stream.length) {
				emitResult("streams", {
		...
		process = (s) => s.remap((emit, chunk) => {
			const row = chunk ? JSON.parse(chunk) : {};
			if (hashLabels(row.labels) !== hashLabels(lastLabel) && stream.length) {
				emitResult("matrix", {
					metric: parseLabels(lastLabel),
					values: stream
				}, !chunk, emit);
			}
			lastLabel = row.labels;
