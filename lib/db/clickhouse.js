...
var initialize = function(dbName){
	...
	var tmp = clickhouse_options;
	tmp.queryOptions.database = '';
	ch = new ClickHouse(tmp);
	ch.query(dbQuery, function (err, data) {
...
const hashLabels = (labels) => {
    return Array.isArray(labels) ? JSON.stringify(labels) : labels;
}
...
const parseLabels = (labels) => {
    return Array.isArray(labels) ? labels.reduce((sum, l) => {
        sum[l[0]] = l[1];
        return sum;
    }, {}) : JSON.parse(labels);
}
...
var scanFingerprints = async function(query, res){
	...
	let _data = '';
	_stream.data.on('data', (data) => {
	    let strData = (_data + data.toString()).split("\n");
	    _data = strData[strData.length - 1];
	    strData.pop();
	    strData.forEach(l => {
	        stream.emit('data', JSON.parse(l));
        });
    });
	_stream.data.on('end', () => {
	    if (_data) {
	...
	//const stream = ch.query(${_query.query});
	stream.on('data', (row) => {
	    if (!_query.matrix) {
            streams[row.fingerprint] = streams[row.fingerprint] || {
                stream: JSON.parse(row.labels),
                values: []
            };
            streams[row.fingerprint].values.push([(parseInt(row.timestamp_ms) * 1000000) + '', row.string]);
            return;
        }
        streams[hashLabels(row.labels)] = streams[hashLabels(row.labels)] || {
            metric: parseLabels(row.labels),
            values: []
        };
        streams[hashLabels(row.labels)].values.push([(parseInt(row.timestamp_ms) / 1000), row.value + ""]);
    });
	stream.on('error', (e) => {
	    console.log(e);
	...
	stream.on('end', () => {
	    if (_query.matrix) {
	        const start = UTILS.parseMs(query.start, Date.now() - 3600 * 1000);
            const end = UTILS.parseMs(query.end, Date.now());
            const step = UTILS.parseOrDefault(query.step, 5);
            Object.values(streams).forEach((s) => {
                let j = 0;
                let _values = [];
                 for (let i = Math.floor(start / 1000); i < Math.floor(end / 1000); i += step) {
                     if (j < s.values.length && i >= s.values[j][0]) {
                         _values.push([i, s.values[j][1]]);
                         while(j < s.values.length && i >= s.values[j][0]) {
                            j++;
                         }
                         continue;
                     }
                     ...
                     _values.push([i, _values[_values.length-1][1]]);
                 }
                 s.values = _values;
            });
        }
	    if (Object.values(streams).length) {
	        resp.data.resultType = _query.matrix ? 'matrix': "streams";
            resp.data.result = Object.values(streams);
        }
        res.send(JSON.stringify(resp));
    });
}
