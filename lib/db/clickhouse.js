...
const outputTempoSearch = async (dataStream, res) => {
  ...
  for await (const item of gen()) {
    ...
    var duration = parseInt((item.Span.end_time_unix_nano - item.Span.start_time_unix_nano) / 1000000) || 0;
    var item = { traceID: item.Span.trace_id, rootServiceName: item.ServiceName, rootTraceName: item.Span.name, startTimeUnixNano: item.Span.start_time_unix_nano, durationMs: duration }
    response += (i ? ',' : '')
