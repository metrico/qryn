...
const samplesReadTable = {
  ...
  check: async () => {
    try {
      logger.info('checking old samples support')
      ...
      samplesReadTable.v1 = tablesResp.data.data.find(row => row.name === 'samples')
      if (!samplesReadTable.v1) {
        return
      }
      const v1EndTime = await axios.post(`${getClickhouseUrl()}/?database=${UTILS.DATABASE_NAME}`,
        'SELECT max(timestamp_ms) as ts FROM samples format JSON')
